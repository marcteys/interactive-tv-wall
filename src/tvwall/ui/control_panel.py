...
class ControlPanelView:
    ...
    def draw(self, state: AppState, rect: Rect) -> None:
        ...
        self.painter.toggle(
            Rect(x, y, 164, 26),
            "Hide Map",
            state.config.hide_maptest,
            PanelAction("toggle_hide_map"),
            tooltip="Show or hide the mapping preview on the right.",
        )
        self.painter.toggle(
            Rect(x + 176, y, 164, 26),
            "Hide Preview",
            state.config.hide_preview,
            PanelAction("toggle_hide_preview"),
            tooltip="Show or hide the shader output preview.",
        )
        y -= 38
        self.painter.toggle(
            Rect(x, y, 164, 26),
            "Fullscreen",
            state.active_monitor.is_fullscreen,
            PanelAction("toggle_fullscreen"),
            tooltip="Toggle fullscreen for the main application window.",
        )
        self.painter.toggle(
            Rect(x + 176, y, 164, 26),
            "Framerate",
            state.show_framerate,
            PanelAction("toggle_framerate"),
            tooltip="Show or hide the live FPS overlay.",
        )
        y -= 38
        ...
        self.painter.button(
            Rect(x + 224, y, 116, 28),
            "Use Testmap",
            PanelAction("use_testmap"),
            tooltip="Switch the source to the generated map view for alignment work.",
        )
        y -= 48
        ...
        self.painter.stepper(
            ...
            PanelAction("adjust_monitor_count", {"delta": 1}),
            tooltip="Set how many monitor configurations exist in the project.",
        )
        y -= 46
        self.painter.segmented(
            ...
            "set_monitor",
            tooltips=[f"Edit settings for monitor slot {index + 1}." for index in range(state.config.number_monitors)],
        )
        y -= 40
        ...
        self.painter.toggle(
            Rect(x, y, 164, 26),
            "Monitor Focus",
            state.config.monitor_focus,
            PanelAction("toggle_monitor_focus"),
            tooltip="Only show TVs assigned to the active monitor in the map preview.",
        )
        self.painter.toggle(
            Rect(x + 176, y, 164, 26),
            "Output Enabled",
            state.active_monitor.output_to_monitor,
            PanelAction("toggle_output_to_monitor"),
            tooltip="Keep this monitor configuration active in the preview/output flow.",
        )
        y -= 38
        self.painter.stepper(
            ...
            PanelAction("adjust_monitor_field", {"field": "tv_number", "delta": 1}),
            tooltip="Choose how many TVs from the global mapping feed this monitor output.",
        )
        y -= 46
        self.painter.stepper(
            ...
            PanelAction("adjust_monitor_field", {"field": "tv_first", "delta": 1}),
            tooltip="Choose the first TV index assigned to this monitor output.",
        )
        y -= 48
        ...
        self.painter.stepper(
            ...
            PanelAction("adjust_canvas", {"field": "canvas_width", "delta": 100}),
            tooltip="Physical width of the full mapped wall, in millimeters.",
        )
        y -= 46
        self.painter.stepper(
            ...
            PanelAction("adjust_canvas", {"field": "canvas_height", "delta": 100}),
            tooltip="Physical height of the full mapped wall, in millimeters.",
        )
        y -= 46
        self.painter.stepper(
            ...
            PanelAction("adjust_tv_count", {"delta": 1}),
            tooltip="Total number of TVs in the installation mapping.",
        )
        y -= 46
        self.painter.stepper(
            ...
            PanelAction("select_tv", {"delta": 1}),
            tooltip="Choose which TV is currently selected for keyboard and field edits.",
        )
        y -= 40
        self.painter.segmented(
            x,
            ...
            state.grid_index,
            "set_grid_index",
            tooltips=[f"Move and resize in {value} mm steps." for value in GRID_OPTIONS],
        )
        y -= 52
        ...
        for tv_index, tv in enumerate(state.config.tv_mappings, start=1):
            ...
            self.painter.button(
                ...
                tooltip=f"Select TV {tv_index} for editing and keyboard nudging.",
            )
            self.painter.stepper(
                x + 92, tv_y, rect.width - 124, "Width", str(tv.width),
                PanelAction("adjust_tv_value", {"index": tv_index, "field": "width", "delta": -10}),
                PanelAction("adjust_tv_value", {"index": tv_index, "field": "width", "delta": 10}),
                tooltip=f"Physical width of TV {tv_index}, in millimeters.",
            )
            tv_y -= 34
            self.painter.stepper(
                ...
                PanelAction("adjust_tv_value", {"index": tv_index, "field": "height", "delta": 10}),
                tooltip=f"Physical height of TV {tv_index}, in millimeters.",
            )
            tv_y -= 34
            self.painter.stepper(
                ...
                PanelAction("adjust_tv_value", {"index": tv_index, "field": "x_pos", "delta": 10}),
                tooltip=f"Horizontal position of TV {tv_index} on the full wall, in millimeters.",
            )
            tv_y -= 34
            self.painter.stepper(
                ...
                PanelAction("adjust_tv_value", {"index": tv_index, "field": "y_pos", "delta": 10}),
                tooltip=f"Vertical position of TV {tv_index} on the full wall, in millimeters.",
            )
            tv_y -= 46
